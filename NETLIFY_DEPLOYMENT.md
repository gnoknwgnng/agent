...
- Supabase Auth + per-user row ownership (`user_id`) for isolation across users
...
- `user_id` on publishing tables
- row-level security (RLS) policies so each authenticated user can only access their own rows
...
6. Enable Supabase Auth email/password (or your preferred provider) for app users.
7. Open `https://YOUR-SITE.netlify.app/publisher.html`.
8. Sign in, then connect LinkedIn and create a schedule.
